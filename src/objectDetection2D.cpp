...
// function definitions for 2D object detection in color images
...
// 2D object detection in color images using YOLO deep-learning framework
void detectObjects(cv::Mat& img, std::vector<BoundingBox>& bBoxes, float confThreshold, 
                   float nmsThreshold, std::string basePath, std::string classesFile,
                   std::string modelConfiguration, std::string modelWeights, bool bVis, bool bWait)
{
    /* detects objects in an image using the YOLO library and a set of pre-trained 
    *  objects from the COCO database; a set of 80 classes is listed in "coco.names" 
    *  and pre-trained weights are stored in "yolov3.weights"
    */
